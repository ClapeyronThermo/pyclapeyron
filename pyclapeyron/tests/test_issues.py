...
def test_issue_140():
    ...
    res = cl.bubble_pressure(model, 280, cl.FractionVector(0.01), 
...
"""
    ...
    model = cl.SAFTgammaMie(["water", "PEG_1sit"], 
                           userlocations=[like_data, assoc_data, mw_data], 
    ...
    assert model.vrmodel.sites.n_sites[2][1] == 1000  # 1000 sites cO_1sit/e1 in PEG.

# @testset "#154" begin
def test_issue_154():
    # there was a problem when using the @newmodel macros outside the Clapeyron module
    # This test verifies that model creation works properly
    # In Python, we don't have macros, so we test model instantiation directly
    assert cl.PCSAFT(["water"]) is not None

# @testset "#162" begin
def test_issue_162():
    # a longstanding problem, init_model didn't worked with functions.
    model1 = cl.Wilson(["water", "ethanol"], puremodel=cl.SRK)
    assert isinstance(model1, cl.EoSModel)
    ...
    assert isinstance(model2, cl.EoSModel)
...
def test_issue_188():
    ...
    file = cl.ParamTable("single", **data, name="db1")
    
    system = cl.PR(["A", "B"], userlocations=[file])
...
def test_issue_201():
    ...
    if hasattr(cl.aspenNRTL, "puremodel"):
        assert isinstance(cl.aspenNRTL(["water", "acetone", "dichloromethane"], 
                                       puremodel=cl.PR), cl.EoSModel)
    else:
        assert isinstance(cl.aspenNRTL(["water", "acetone", "dichloromethane"]), 
                         cl.EoSModel)
    
    assert isinstance(cl.UNIFAC(["water", "acetone", "dichloromethane"]), 
                     cl.EoSModel)
...
def test_SorptionModels_init_kij():
    ...
    model1 = cl.SL(
        ["PC", "CO2"],
        userlocations={
            "vol": [v_star(P_star[i], T_star[i]) for i in range(2)],
            "segment": [r(P_star[i], T_star[i], rho_star[i], mw[i]) for i in range(2)],
            "epsilon": [epsilon_star(T_star[i]) for i in range(2)],
            "Mw": mw
        },
...
def test_discussion_239():
    ...
    m2 = cl.CPA(["Methanol"], userlocations={
        ...
        "n_H": [1],
        "n_e": [1],
        "epsilon_assoc": {(("Methanol", "H"), ("Methanol", "e")): m1.params.epsilon_assoc.values.values[1]},
        "bondvol": {(("Methanol", "H"), ("Methanol", "e")): m1.params.bondvol.values.values[1]}
    })
...
def test_infinite_dilution_derivatives_association():
    ...
    cp_params = {
        "a": [36.54206320678348, 39.19437678197436, 25.7415, 34.91747774761048],
        "b": [-0.03480434051958945, -0.05808483585041852, 0.2355, -0.014935581577635826],
        "c": [0.000116818199785053, 0.0003501220208504329, 0.0001578, 0.000756101594841365],
        "d": [-1.3003819534791665e-7, -3.6941157412454843e-7, -4.0939e-7, -1.0894144551347726e-6],
        "e": [5.2547403746728466e-11, 1.276270011886522e-10, 2.1166e-10, 4.896983427747592e-10]
    }
    ...
    cp_pure = cl.VT_isobaric_heat_capacity(water, v, 298.15, 1.0)
    cp_mix = cl.VT_isobaric_heat_capacity(pcpsaft, v, 298.15, z)
...
def test_CPA_init():
    ...
    assert isinstance(model1, cl.CPA)
