...
def test_Bulk_properties_single():
    ...
    assert cl.pip(system, v, T) == approx(6.857076349623449, rel=1e-6)
    assert cl.is_liquid(cl.VT_identify_phase(system, v, T))
    ...
    assert cl.VT_entropy_res(system, v, T) + cl.VT_entropy(cl.idealmodel(system), v, T) == approx(s)
    ...
    assert cl.VT_internal_energy_res(system, v, T) + cl.VT_internal_energy(cl.idealmodel(system), v, T) == approx(u)
    
    h = cl.enthalpy(system, p, T)
    assert h == approx(-35876.32155687084, rel=1e-6)
    assert cl.VT_enthalpy_res(system, v, T) + cl.VT_enthalpy(cl.idealmodel(system), v, T) == approx(h)
    
    g = cl.gibbs_free_energy(system, p, T)
    assert g == approx(-18323.87754268292, rel=1e-6)
    assert cl.VT_gibbs_free_energy_res(system, v, T) + cl.VT_gibbs_free_energy(cl.idealmodel(system), v, T) == approx(g)
    
    a = cl.helmholtz_free_energy(system, p, T)
    assert a == approx(-18329.785451419295, rel=1e-6)
    assert cl.VT_helmholtz_free_energy_res(system, v, T) + cl.VT_helmholtz_free_energy(cl.idealmodel(system), v, T) == approx(a)
...
def test_Bulk_properties_multi():
    ...
    assert cl.mixing(system, p, T, z, cl.gibbs_free_energy) == approx(-178.10797973342596, rel=1e-6)
    assert cl.excess(system, p, T, z, cl.volume) == approx(1.004651584989827e-6, rel=1e-6)
    assert cl.excess(system, p, T, z, cl.entropy) == approx(-2.832281578281112, rel=1e-6)
    assert cl.excess(system, p, T, z, cl.gibbs_free_energy) == approx(1626.6212908893858, rel=1e-6)
...
def test_Equilibrium_properties_multi():
    ...
    res_LLE_p = cl.LLE_pressure(system, T, z2)
    ...
    res_LLE_T = cl.LLE_temperature(system, p, z2)
    T_LLE = res_LLE_T[0]
    ...
    gc.collect()
    assert cl.UCST_mix(system, T2)[0] == approx(1.0211532467788119e9, rel=1e-6)
    gc.collect()
