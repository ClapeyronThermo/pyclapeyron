...
def test_margules():
    ...
    p = 1e5
    z = [0.5, 0.5]
    system = cl.Margules(["methanol", "water"])
    assert cl.excess_gibbs_free_energy(system, p, T, z) == approx(cl.test_excess_gibbs_free_energy(system, p, T, z), rel=1e-6)
...
def test_van_laar():
    ...
    p = 1e5
    z = [0.5, 0.5]
    system = cl.VanLaar(["methanol", "water"])
    assert cl.excess_gibbs_free_energy(system, p, T, z) == approx(cl.test_excess_gibbs_free_energy(system, p, T, z), rel=1e-6)
...
def test_wilson():
    ...
    assert cl.activity_coefficient(system, p, T, z) == approx(cl.test_activity_coefficient(system, p, T, z), rel=1e-6)
    assert cl.excess_gibbs_free_energy(system, p, T, z) == approx(cl.test_excess_gibbs_free_energy(system, p, T, z), rel=1e-6)
...
def test_unifac2():
    ...
    p = 1e5
    z = [0.5, 0.5]
    system = cl.UNIFAC2([("acetaldehyde", {"CH3": 1, "HCO": 1}), ("acetonitrile", {"CH3CN": 1})], puremodel=cl.BasicIdeal())
    assert np.log(cl.activity_coefficient(system, float('nan'), 323.15, [0.5, 0.5])[0]) == approx(0.029527741236233, rel=1e-6)
...
def test_og_unifac2():
    ...
    p = 1e5
    z = [0.5, 0.5]
    system = cl.ogUNIFAC2([("R22", {"HCCLF2": 1}), ("carbon disulfide", {"CS2": 1})], puremodel=cl.BasicIdeal())
    assert np.log(cl.activity_coefficient(system, float('nan'), 298.15, [0.3693, 0.6307])[0]) == approx(0.613323250984226, rel=1e-6)
...
def test_cosmosac02():
    ...
    assert cl.excess_gibbs_free_energy(system, p, T, z) == approx(610.5706657776052, rel=1e-6)
...
def test_joback():
    ...
    assert cl.VT_isobaric_heat_capacity(system, V, 298.15) == approx(143.22076150138616, rel=1e-6)
    assert cl.crit_pure(system)[0] == approx(500.2728274871347, rel=1e-6)
    assert cl.a_ideal(system, V, T, z) == approx(9.210841420941021, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
    assert cl.mass_density(system, p, T, z) == approx(cl.molecular_weight(system, z) * p / (cl.Rgas(system) * T))
    ...
    assert cl.JobackGC.T_b(s0)[0] == approx(322.1100, rel=1e-6)
...
def test_reid():
    ...
    assert cl.a_ideal(system, V, T, z) == approx(9.210842104089576, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
    assert cl.mass_density(system, p, T, z) == approx(cl.molecular_weight(system, z) * p / (cl.Rgas(system) * T))
...
def test_shomate():
    ...
    coeff = system.params.coeffs[0]
    assert cl.evalcoeff(system, coeff, 500) == approx(35.21836175, rel=1e-6)
    assert cl.eval_integral_coeff(system, coeff, 500) == approx(15979.2447, rel=1e-6)
    assert cl.eval_integral_coeff_T(system, coeff, 500) == approx(191.00554, rel=1e-6)
    assert cl.mass_density(system, p, T, z) == approx(cl.molecular_weight(system, z) * p / (cl.Rgas(system) * T))
...
def test_walker():
    ...
    assert cl.molecular_weight(system) * 1000 == approx(86.21)
    assert cl.a_ideal(system, V, T, z) == approx(179.51502015696653, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
    assert cl.mass_density(system, p, T, z) == approx(cl.molecular_weight(system, z) * p / (cl.Rgas(system) * T))
    ...
    """
    system2 = cl.WalkerIdeal([("ideal gas", {"ideal": 1})], userlocations=[ideal_csv])
    assert cl.isobaric_heat_capacity(system2, 1e5, T) / cl.Rgas() == approx(2.5)
...
def test_monomer():
    ...
    assert cl.a_ideal(system, V, T, z) == approx(-10.00711774776317, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
    assert cl.mass_density(system, p, T, z) == approx(cl.molecular_weight(system, z) * p / (cl.Rgas(system) * T))
...
def test_empiric():
    ...
    assert cl.a_ideal(system, V, T, z) == approx(7.932205569922042, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
    assert cl.mass_density(system, p, T, z) == approx(cl.molecular_weight(system, z) * p / (cl.Rgas(system) * T))
    
    # Empiric Ideal from already existing MultiFluid model
    system = cl.idealmodel(cl.MultiFluid(["water"]))
    assert cl.a_ideal(system, V, T, z) == approx(7.932205569922042, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
    
    # Empiric Ideal from already existing single fluid model
    system = cl.idealmodel(system.pures[0])
    assert cl.a_ideal(system, V, T, z) == approx(7.932205569922042, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
...
def test_aly_lee():
    ...
    # assert cl.a_ideal(system, V, T, z) == approx(9.239701647126086, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
    assert cl.mass_density(system, p, T, z) == approx(cl.molecular_weight(system, z) * p / (cl.Rgas(system) * T))
    
    # we use the default GERG 2008 parameters for methane, test if the Cp is equal
    system_gerg = cl.idealmodel(cl.GERG2008(["methane"]))
    Cp_system = cl.VT_isobaric_heat_capacity(system, V, T, z)
    Cp_gerg = cl.VT_isobaric_heat_capacity(system_gerg, V, T, z)
...
def test_cp_lng_estimation():
    ...
    system = cl.CPLNGEstIdeal(["a1"], userlocations={'Mw': [20.5200706797]})
    # test at 324.33 K, paper says Cp = 44.232, but the calculations in the paper seem off
    assert cl.VT_isobaric_heat_capacity(system, 0.03, 324.33) == approx(44.231, rel=5e-4)
...
def test_ppds():
    ...
    assert cl.mass_density(m1, p, T, z) == approx(cl.molecular_weight(m1, z) * p / (cl.Rgas(m1) * T))
    mw2 = 32.042
...
def test_iapws95():
    ...
    z = np.array([1.0])
    system = cl.IAPWS95()
    system_ideal = cl.idealmodel(system)
    assert cl.a_ideal(system_ideal, V, T, z) == approx(7.9322055699220435, rel=1e-6)
    assert cl.a_ideal(system, V, T, z) == approx(7.9322055699220435, rel=1e-6)
    assert cl.a_res(system, V, T, z) == approx(-2.1152889226862166e14, rel=1e-6)
    # because we are in this regime, numerical accuracy suffers. that is why big(V) is used instead.
    # assert cl.ideal_consistency(system, big(V), T, z) == approx(0.0, abs=1e-14)
...
def test_propane_ref():
    ...
    assert cl.a_ideal(system, V, T, z) == approx(0.6426994942361217, rel=1e-6)
    assert cl.a_res(system, V, T, z) == approx(-2.436280448227229, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z) == approx(0.0, abs=1e-14)
...
def test_gerg2008():
    ...
    assert cl.a_ideal(system, V, T, z) == approx(4.500151936577565, rel=1e-6)
    assert cl.a_res(system, V, T, z) == approx(-10.122119572808764, rel=1e-6)
    z4 = [0.25, 0.25, 0.25, 0.25]
    system = cl.GERG2008(["water", "carbon dioxide", "hydrogen sulfide", "argon"])
    assert cl.a_ideal(system, V, T, z4) == approx(3.1136322215343917, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z4) == approx(0.0, rel=1e-14)
    assert cl.a_res(system, V, T, z4) == approx(-1.1706377677539772, rel=1e-6)
    assert cl.ideal_consistency(system, V, T, z4) == approx(0.0, abs=1e-14)
...
def test_eos_lng():
    ...
    dep = cl.departure_functions(system)
    assert sum(1 for x in dep if x != 0) == 1
...
def test_lkp():
    ...
    assert cl.a_res(system, Vx, Tx, zx) == approx(-6.469596957611441, rel=5e-6)
    system2 = cl.LKPSJT(["methane", "butane"])
    # TODO: check this value
    assert cl.a_res(system2, Vx, Tx, zx) == approx(-5.636923220762173, rel=5e-6)
...
def test_lj_ref():
    ...
    Tx = 1.051 * cl.T_scale(system)
    Vx = cl._v_scale(system) / 0.673
    assert cl.a_ideal(system, Vx, Tx) == approx(5.704213386278148, rel=1e-6)
    assert cl.a_res(system, Vx, Tx) == approx(-2.244730279521925, rel=1e-6)
    assert cl.ideal_consistency(system, Vx, Tx, z1) == approx(0.0, abs=1e-14)
...
def test_xiang_deiters():
    ...
    # equal to cl.a_ideal(cl.BasicIdeal(["water"]), V, T, z)
    assert cl.a_ideal(system, V, T, z1) == approx(-0.33605470137749016, rel=1e-6)
    assert cl.a_res(system, V, T) == approx(-34.16747927719535, rel=1e-6)
...
def test_multiparameter_misc():
    ...
    pures = cl.split_pure_model(model)
    assert isinstance(pures, list)
    assert cl.wilson_k_values(model, 1e6, 300.0) == approx([6.738566125478432, 54.26124873240438], rel=1e-3)
    assert cl.a_res(model, V, T, z2) == approx(-0.005482930754339683, rel=1e-6)
    
    model2 = cl.SingleFluid("ammonia", verbose=True)
    assert cl.a_res(model2, V, T, z1) == approx(-0.05006143389915488, rel=1e-6)
    
    model3 = cl.SingleFluid("D4", verbose=True)
    assert cl.a_ideal(model3, V, T, z1) == approx(0.8441669238992482, rel=1e-6)
    
    model4 = cl.SingleFluid("R14", verbose=True)
    assert cl.a_res(model4, V, T, z1) == approx(-0.017855323645451636, rel=1e-6)
...
def test_sanchez_lacombe_single():
    ...
    assert cl.a_res(system, V, T, z) == approx(-0.9511044462267396, rel=1e-6)
...
def test_sanchez_lacombe_kij_rule():
    ...
    assert cl.a_res(system, V, T, z2) == approx(-6.494291842858994, rel=1e-6)
...
def test_sanchez_lacombe_k0k1l():
    ...
    assert cl.a_res(system, V, T, z2) == approx(-5.579621796375229, rel=1e-6)
...
def test_costald():
    ...
    assert cl.volume(system2, 1e5, 300.15, [0.5, 0.5]) == approx(2.834714146558056e-5, rel=1e-6)
    assert cl.volume(system2, 1e5, 300.15, [1., 0.]) == approx(1.8553472145724288e-5, rel=1e-6)
...
def test_dippr105_liquid():
    ...
    assert cl.volume(system2, 1e5, 300.15, [0.5, 0.5]) == approx(2.9367802477146567e-5, rel=1e-6)
    assert cl.volume(system2, 1e5, 300.15, [1., 0.]) == approx(1.8057164858551208e-5, rel=1e-6)
...
def test_rackett_liquid():
    ...
    assert cl.volume(system2, 1e5, 300.15, [0.5, 0.5]) == approx(3.2516352601748416e-5, rel=1e-6)
    assert cl.volume(system2, 1e5, 300.15, [1., 0.]) == approx(1.6837207241594103e-5, rel=1e-6)
...
def test_yamada_gunn_liquid():
    ...
    assert cl.volume(system2, 1e5, 300.15, [0.5, 0.5]) == approx(3.825994563177142e-5, rel=1e-6)
    assert cl.volume(system2, 1e5, 300.15, [1., 0.]) == approx(2.0757546189420953e-5, rel=1e-6)
...
def test_grenke_elliott_water():
    ...
    for Ti in np.linspace(250.0, 280.0, 30):
        ...
        Cpi_test = cl.water_cp(system, Ti)
...
def test_holten_water():
    ...
    for i in range(5):
        ...
        L = cl.water_L(model, p_scaled, T)
        omega = 2 + 0.5212269 * p_scaled
        xe = cl.water_x_frac(model, L, omega)
...
def test_abbott_virial():
    ...
    assert cl.volume(system, 1e5, 300, [0.5, 0.5]) == approx(0.024820060368027988, rel=1e-6)
    assert cl.a_res(system, 0.05, 300, [0.5, 0.5]) == approx(-0.0024543543773067693, rel=1e-6)


def test_tsonopoulos_virial():
    system = cl.TsonopoulosVirial(["methane", "ethane"])
    assert cl.volume(system, 1e5, 300, [0.5, 0.5]) == approx(0.02485310667780686, rel=1e-6)
    assert cl.a_res(system, 0.05, 300, [0.5, 0.5]) == approx(-0.0017990881811592349, rel=1e-6)
...
def test_eos_virial2():
    ...
    assert cl.volume(system, 1e5, 300, [0.5, 0.5]) == cl.volume_virial(cub, 1e5, 300, [0.5, 0.5])
    assert cl.a_res(system, 0.05, 300, [0.5, 0.5]) == approx(-0.0023728381262076137, rel=1e-6)


def test_solid_hfus():
    model = cl.SolidHfus(["water"])
    assert cl.chemical_potential(model, 1e5, 298.15, [1.])[0] == approx(549.1488193300384, rel=1e-6)
...
def test_iapws06():
    ...
    p2, T2 = 101325.0, 273.152519
    p3, T3 = 100e6, 100.0
    Mw = cl.molecular_weight(model)
    assert cl.mass_gibbs_energy(model, p1, T1) == approx(0.611784135, rel=1e-6)
    assert cl.mass_gibbs_energy(model, p2, T2) == approx(0.10134274069e3, rel=1e-6)
    assert cl.mass_gibbs_energy(model, p3, T3) == approx(-0.222296513088e6, rel=1e-6)
...
def test_jager_span_solid_co2():
    ...
    p2, T2 = 100e6, 100.0
    assert cl.gibbs_energy(model, p1, T1) == approx(-1.447007522e3, rel=1e-6)
    assert cl.gibbs_energy(model, p2, T2) == approx(-2.961795962e3, rel=1e-6)
