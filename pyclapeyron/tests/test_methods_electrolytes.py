...
def test_Solid_solubility():
    ...
    assert cl.sle_solubility(system, p, 270., [1, 1, 1], solute=["water"])[0] == approx(0.9683019351679348, rel=1e-6)
    assert cl.sle_solubility(system, p, 300., [1, 1, 1], solute=["sodium.chloride"], x0=[-1.2])[0] == approx(0.7570505178871523, rel=1e-6)
...
def test_Tp_flash():
    ...
    x, n, G = cl.tp_flash(system, p, T, [0.4, 0.4, 0.1, 0.1], 
                          cl.MichelsenTPFlash(equilibrium="lle", K0=[100., 1e-3, 1000., 1000.]))
    # This test is marked as broken in Julia
    pytest.skip("Test broken - x[0, 0] ≈ 0.07303405420993273 rtol = 1e-6")
